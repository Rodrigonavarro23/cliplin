...
- `src/cliplin/utils/`: Shared functionality used by both CLI and MCP: chromadb (client, collections, document ops), templates, ai_host_integrations (one class per AI host, see docs/tdrs/ai-host-integration-handler-pattern.md), tools; add fingerprint/change-detection here so it is reusable
...
- Context directories and collection mappings (docs/adrs, docs/business, docs/features, docs/tdrs, docs/ts4, docs/ui-intent → business-and-architecture, features, technical-decision-records, tech-specs, uisi; plus .cliplin/knowledge/** per docs/tdrs/knowledge-reindex-context.md) must be defined in one place (e.g. COLLECTION_MAPPINGS in utils/chromadb.py or a dedicated config module)
