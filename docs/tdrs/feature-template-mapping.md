...
- docs/features/cli.feature ↔ create_framework_knowledge_package in src/cliplin/utils/templates.py — creates the built-in framework package at .cliplin/knowledge/cliplin-framework/ with framework ADRs (000, 001, 002, 005). Any change to init scenarios (e.g. framework package location, ADR list) MUST be reflected in the template.
- docs/features/context-audit.feature ↔ create_context_audit_skill in src/cliplin/utils/templates.py — creates `.cliplin/knowledge/cliplin-framework/skills/cliplin-context-audit/SKILL.md` with the full skill content (JSON schema, context_score formula, audit instructions). Any change to context-audit scenarios (e.g. new output field, new scoring rule, new linking behavior) MUST be reflected in create_context_audit_skill. See docs/tdrs/context-audit-skill.md for the governing spec.
- docs/features/reverse-engineer.feature ↔ create_reverse_engineer_skill in src/cliplin/utils/templates.py — creates `.cliplin/knowledge/cliplin-framework/skills/cliplin-reverse-engineer/SKILL.md` with the full skill content (scan sources, module detection, two-phase guidance flow, tracking file, resume behavior). Any change to reverse-engineer.feature scenarios (e.g. new scan source, new phase behavior, new tracking field) MUST be reflected in create_reverse_engineer_skill. See docs/tdrs/reverse-engineer-skill.md for the governing spec.
- (Add more mappings here when features gain template reflections.)

## Verification workflow (MUST when modifying a mapped feature)

1. After modifying a .feature file, check docs/tdrs/feature-template-mapping.md for that feature.
...
- Add the feature ↔ template mapping to this TS4 (known mappings section).
- Document the verification requirement in .cursor/rules/feature-processing.mdc and docs/tdrs/cross-references-on-evolution.md.
code_refs:
  - "docs/features/"
