...
- ALWAYS pass an absolute, resolved path to the client: `path=str(db_path.parent.resolve())` (critical for Windows; see docs/tdrs/windows-compatibility-file-operations.md)
...
- Collection names must match the central mapping (e.g. business-and-architecture, features, tech-specs, uisi) when indexing project context files
...
- Use file path relative to project root as document ID for context files (e.g. `docs/tdrs/my-spec.md`) to enable upserts and avoid duplicates
- Include metadata for every indexed document: at least `file_path` (relative path), `type` (adr|project-doc|feature|ts4|ui-intent), `collection` (target collection name)
...
- Use UTF-8 encoding for reading and writing the fingerprint store file; see docs/tdrs/windows-compatibility-file-operations.md
...
code_refs:
  ...
  - "docs/tdrs/windows-compatibility-file-operations.md"
